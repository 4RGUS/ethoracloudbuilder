...
//function to remove files from a given directory
function removeFiles(filesToDelete) {
  for (let i = 0; i < filesToDelete.length; i++) {
    fs.unlink(filesToDelete[i], (err) => {
      if (err) throw err;
      console.log(`File ${filesToDelete[i]} deleted`);
    });
  }
}
...
function createCustomConfig(data) {
  ...
  */
  ...
  const appTitle = "${appTitle || "Ethora"}"; //done
  // LOGO (LOGIN SCREEN)
  ...
  */
  ...
  const coinReplacedSymbol = "${coinSymbol || "ETO"}"; //done
  const coinReplacedName = "${coinName || "Ethora Coin"}"; //done
